...
to androids-wander
  ...
  ;[
    ask androids
    [
      set heading heading + (random 30 - 15)
      ;face one-of neighbors4
      let ad android-delay
      if (android-delay = 0) [ set ad .05 ]
      let go-fwd (.1 / ad)
      if ( patch-ahead (go-fwd) = nobody )  [ set heading heading + 180 ]
      fd go-fwd
   ; ]
  ]
end
...
to send-sick-numbers
  hubnet-send user-id "Turtles" count turtles
  hubnet-send user-id "Number Sick" count turtles with [infected? = true]
...
"run 1" 1.0 0 -2674135 true "" ""
"run 2" 1.0 0 -10899396 true "" ""
