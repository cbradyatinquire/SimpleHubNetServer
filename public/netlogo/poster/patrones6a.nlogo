...
globals [ index hostname status last-state]
...
to setup
  ...
  set hostname "abmplus.tech.northwestern.edu"
   ...
   crt 1 [ set shape "line half" set color white set size 31 setxy  22 -1 set heading 180 ]
   crt 1 [ set shape "line half" set color white set size 31 setxy  28 -1 set heading 180 ]
   
   crt 1 [ set shape "line half" set color white set size 31 setxy  17.5 -3.5 set heading 90 ]
   ...
   [
     ...
     set id -1 * ind
     set label-color white
     setxy 32 -.1 + -2.5 * (ind + 1)
     ifelse ind = 0 [ set label "CAMB"]
     [ set label ( "  ") set ycor ycor - 1.4 if ind = 5 [ die ] ]
   ]
   set ind ind + 1
   ]
end
...
to dibujar
  every .001
  [
  ifelse (mouse-down? and last-state = "up")
  [
    set last-state "down"
  ...
  [
    if ( not mouse-down? ) [ set last-state "up" ]
  ]
  ]
end
...
to analizar
  ...
  let show-camb false
  let p1 (count patches with [pcolor = yellow])
  let p2 (count patches with [pcolor = red])
  let p3 (count patches with [pcolor = violet])
  let p4 (count patches with [pcolor = green])
  let p5 (count patches with [pcolor = sky])
  let d1 ""
  let d2 ""
  let d3 ""
  let d4 ""
  if (p1 > 0 and p2 > 0 and p3 > 0 and p4 > 0 and p5 > 0 ) [ set show-camb true ]
  if (show-camb = true)
  [
   set d1 (p2 - p1)
   set d2 (p3 - p2)
  ...
  ask signs 
  [
    if id = 1 [ set label (word p1 "  ")]
    if id = 2 [ set label (word p2 "  ")]
    if id = 3 [ set label (word p3 "  ")]
    if id = 4 [ set label (word p4 "  ")]
    if id = 5 [ set label (word p5 "  ")]
    
    if id = -1 [ set label (word d1 "   ")]
    if id = -2 [ set label (word d2 "   ")]
    if id = -3 [ set label (word d3 "   ")]
    if id = -4 [ set label (word d4 "   ")]
